...
(async()=>{const cfg=JSON.parse(fs.readFileSync('tmp/m3-05-test.json'));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
  ...
  const summary=page.locator('#v2-management-summary');const governance=page.locator('#v2-governance-card');await governance.filter({hasText:'数据治理基线'}).waitFor();await governance.filter({hasText:'M309缺成本商品'}).waitFor();await governance.locator('code').first().waitFor();
  assert.match(await governance.locator('code').first().innerText(),/^gov_[0-9a-f]{20}$/);assert.match(await governance.innerText(),/影响金额.*问题 ID.*优先级/s);
  const [download]=await Promise.all([page.waitForEvent('download'),governance.getByRole('button',{name:'导出治理清单'}).click()]);
