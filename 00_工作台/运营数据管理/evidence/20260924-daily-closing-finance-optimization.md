...
## rc.8 日结收入明细导出（已发布待观察）

日结报表的历史月份区域新增“导出收入明细”。该入口直接复用“数据管理 → 导出收入数据”的收入事实加载、CSV 字段和生成逻辑，仅将日期范围固定为所选历史月份。导出字段保持：日期、来源、分类、收入金额、净收入、收款方式、项目/关联、经手人、原记录 ID、创建时间；收入继续覆盖收银台、空间和画廊。

专项浏览器回归以 viewer 权限保持页面查看 9 月、选择 8 月，验证导出文件名为 `艾维美术馆_收入_2026-08-01_2026-08-31.csv`，包含 8 月收入、不包含 9 月收入，字段与数据管理导出一致。日结 Excel、存现金、弹窗明细和移动端回归同时通过。未新增数据管理权限、收入写权限或数据库结构。

`2.0.0-rc.8` 已于 2026-09-24 发布生产。数据库备份 `/opt/aiwei/backups/postgres/aiwei-postgres-20260924-170323.dump` 为 406,477 bytes，恢复清单校验通过；静态回滚点为 `/opt/aiwei/backups/daily-closing-income-export-20260924-1704`。五个发布文件本地与远端 SHA-256 一致，HTTPS 首页、三个脚本和健康检查返回 200，未登录收入接口返回 401，容器健康且 API 错误扫描为空。发布窗口内基线新增 1 笔收入，审计日志为同时间的 `create:revenue`，无更新、删除、结构或配置变化，判定为正常营业并发写入；灰度保持 `staff`，未关闭 P0/P1 为 0。
