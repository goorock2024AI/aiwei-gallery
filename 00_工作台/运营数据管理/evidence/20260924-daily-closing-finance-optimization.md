...
专项浏览器回归以 viewer 权限保持页面查看 9 月、选择 8 月，验证导出文件名为 `艾维美术馆_收入_2026-08-01_2026-08-31.csv`，包含 8 月收入、不包含 9 月收入，字段与数据管理导出一致。日结 Excel、存现金、弹窗明细和移动端回归同时通过。未新增数据管理权限、收入写权限或数据库结构；`2.0.0-rc.8` 当前尚未部署生产。
