## 2026-09-06 M3-06 工坊与体验活动本地交付

版本 2.0.0-dev.m3-06.1。工坊收入新增活动项目、工坊/课程类型、参与人数快照，并按月关联 M3-05 已确认的体验活动直接成本，展示收入、直接成本、贡献额及待补状态。19 项接口与 SQL 检查、跨月边界、旧明细兼容、迁移回滚/幂等、桌面和手机实际收款及既有专项回归通过。两处 dist 同步；未生产迁移或部署。详见 2.0 开发文档 M3-06 与 evidence/20260906-m3-06-verification.md。下一开发任务 M3-07。
...
- 柜台存现金后仍可在日结台账和当日明细中追溯金额，支持财务核对柜台现金减少原因。
- 日结“查看”不再把明细堆叠在页面底部，改为独立弹窗。
- 财务只读账号可按所选月份导出 Excel 日结报表，同时不获得日结写入、复核或现金流水写入权限。

**改动**

- 月度日结台账新增“存现金”列，数据直接来自 `cash_movements.type = cash_deposit`，存现只作为资金转移，不重复计入收入。
- 日结查看改为最大 1120px、可滚动且适配 390px 手机宽度的模态窗口；保存后继续在弹窗内刷新。
- 新增“导出本月日结”，输出 `.xlsx`，按日包含状态、系统净收入、实收、差异、品类、收款方式、柜台现金期初/收款/存现/其他流出/期末、结账人、复核人和备注。
- editor/viewer 前端仅新增 `daily-closing` 导出权限；viewer 后端仅新增 `cash_movements` 读取权限，写权限矩阵未扩大。
- 发布候选版本为 `2.0.0-rc.6`；同步项目 `dist/` 与根目录 `dist/` 静态镜像，缓存标识为 `2.0.0-rc.6-20260924`。

**验证**

- `node --check app/js/ui.js`、`app/js/auth.js`、`server.js` 通过。
- `npm run check:daily-closing-finance-browser` 通过：核对存现 80 元在台账和弹窗可见，Excel 中现金期初 50、现金收款 100、存现 80、期末 70，viewer 只能导出，390px 弹窗不越界。
- `npm run check:dist-sync`、`scripts/check-v2-workspace.js`、`git diff --check` 通过。
- `npm run check:m6-core` 通过 104 项 HTTP 检查：未登录、viewer、editor、admin 权限矩阵，viewer 现金流水只读、日结复核边界及原始事实恢复均通过。
- `check:m6-release` 当前按设计因工作区存在本次未提交改动而拒绝，不属于功能失败。
...
**历史月份导出修正（`2.0.0-rc.7` 已发布待观察）**

- 新增独立“历史导出月份”，页面日结日期只负责查看，Excel 导出严格使用所选月份。
- 跨月份回归保持查看 9 月并导出 8 月，查询区间、文件名、月份数据隔离和现金等式全部通过。
- 无数据库迁移、经营数据写入或权限变化。已生成 405,502-byte 数据库备份与独立静态回滚点后发布；四个线上文件哈希、HTTPS、鉴权、容器、日志及发布前后业务基线复核通过，灰度保持 `staff`，未关闭 P0/P1 为 0。

**收入明细导出（`2.0.0-rc.8` 已发布待观察）**

- 日结报表增加“导出收入明细”，按“历史导出月份”生成 CSV。
- 直接复用数据管理收入导出的收入事实源、十个字段及 CSV 逻辑，覆盖收银台、空间和画廊，不维护第二套导出口径。
- viewer 专项验证跨月份数据隔离、文件名、字段和内容通过；未增加数据管理访问、收入写入或其他导出权限。
- 已生成 406,477-byte 数据库备份和独立静态回滚点后发布；五个文件哈希、HTTPS、鉴权、容器和日志通过。发布窗口内新增 1 笔收入由同时间 `create:revenue` 审计完整解释，无更新、删除、结构或配置变化；灰度保持 `staff`，未关闭 P0/P1 为 0。

---

## 2026-09-20 九月全部收费项目成本同步补算

**需求编号**：OPS-FIN-20260920-02
**状态**：已发布待观察

- 计划版本 `2.0.0-rc.5`，三套静态入口和版本标识已同步。
- 修正范围为门票、套票、咖啡、工坊全部收费项目；2026-09-01 起每条销售均生成成本事实。
- 正数成交快照优先；无快照或快照为 0 时读取产品管理当前成本。成本仍为 0 表示尚未补录，进入缺成本治理清单；补录后历史毛利自动重算。
- 生产只读盘点确认套票 182 份可补成本 ¥819，咖啡 20 杯可补成本 ¥94.50；普通票和两项已售工坊当前仍待补成本。
- 原始 `revenue` 数据不改写；专项测试覆盖四类商品、补录联动、快照优先、缺成本、幂等与回滚。
- M6 全链保持 22 个正向迁移、18 个回滚迁移，旧经营事实不变。
- 证据：`evidence/20260920-september-combo-cost-period.md`。

**生产发布**

- 版本 `2.0.0-rc.5`；提交 `acf8204` 已推送 `codex/operations-v2`。
- 备份 `/opt/aiwei/backups/postgres/aiwei-postgres-20260920-143350.dump`，双 SHA 与恢复列表验证通过；静态回滚目录 `/opt/aiwei/backups/configured-cost-sync-20260920-1435`。
- 9 月套票补算 ¥819、咖啡补算 ¥94.50；到馆参观毛利变为 ¥6,921。普通票和两项已售工坊保留缺成本提示，补录后自动重算。
- 四类事务冒烟、补录联动、快照优先和回滚清理通过；测试记录 0 残留，临时脚本已删除。
- 公网 HTTPS、版本标识、文件 SHA、API 健康、容器、错误日志和未登录 401 全部通过。

---

## 2026-09-20 配置商品成本进入毛利口径

**需求编号**：OPS-FIN-20260920
**状态**：已发布待观察

**修复**

- 门票、套票、咖啡、工坊在成交时冻结单位成本，并写入销售明细快照。
- 编辑已有流水时保留成交时成本；旧流水没有成本证据时保持为 0，不用当前配置倒推历史。
- 新增配置商品销售成本事实，统一进入单笔利润、四层月度销售成本、销售毛利和零成本治理清单。
- 新增正向/回滚迁移并纳入 M6 完整迁移清单；不改写收入、支出或历史经营流水。
- 前端缓存标识更新为 `configured-sale-cost-20260920`，三套静态文件已同步。

**验证**

- 配置商品成本单元和数据库集成测试通过，覆盖四类商品、历史不回填、零成本 P1、幂等和回滚。
- POS 门票浏览器回归通过，覆盖新成交成本快照与历史编辑保护。
- 产品成本维护、工坊项目、文创快照、静态同步和 JS 语法检查通过。
- M6 新库演练通过：21 个正向迁移、17 个回滚迁移、完整回滚/重放，旧经营事实不变。
- 开发阶段标准预检先被“工作区必须干净”门禁拦截；提交后标准发布预检在干净工作区通过，迁移清单、校验和与静态镜像均正确。

**边界与回滚**

- 回滚脚本：`sql/20260920_configured_sale_cost_snapshots_rollback.sql`。
- 详细证据：`evidence/20260920-configured-sale-cost-gross-profit.md`。

**生产发布**

- 发布版本：`2.0.0-rc.4`；提交：`5e6f431`，已推送 `codex/operations-v2`。
- 发布前数据库备份：`/opt/aiwei/backups/postgres/aiwei-postgres-20260920-131536.dump`，385,398 bytes；dump、基线 SHA 和 `pg_restore --list` 均通过。
- 代码回滚目录：`/opt/aiwei/backups/configured-sale-cost-20260920-131800`。
- 生产迁移事务成功，收入/支出原始事实指纹前后一致；历史配置商品成本事实为 0 行，未自动回填历史。
- 四品类事务内冒烟得到销售成本 6 / 7 / 8 / 60，三层毛利 32 / 22 / 240；事务回滚后测试记录为 0。
- 线上四个静态资源 HTTP 200，版本和成本快照标识正确；本地与远端 8 个发布文件 SHA-256 一致。
- API、数据库、Nginx 均健康，内部 `/healthz` 正常，API 日志错误命中 0；未登录收入和成本事实接口均返回 401。
- 不回填发布前缺少成本快照的历史流水；如需历史补录，必须另行确认有效期间和成本证据。

---

## 2026-09-05 M3-05 支出与成本归属本地交付

版本 2.0.0-dev.m3-05.1。完成成本类型、业务层、能力轴、项目关联、默认建议与待归类筛选；原子保存原支出、归属链接和服务端审计，保留原类别及期间支出口径。30 项 HTTP 检查、真实事务失败回退、迁移回滚/幂等、桌面和手机保存及原有专项回归通过。两处 dist 同步，未生产迁移或部署。六步记录、证据及回退详见 2.0 开发文档 M3-05 与 evidence/20260905-m3-05-verification.md。下一开发任务 M3-06。

---

## 2026-09-05 M3-04 第二批本地交付

完成 POS 商品/成本快照、JSONB 编辑兼容、商品别名维护、快照事实视图；同步两处 dist，版本 2.0.0-dev.m3-04.2。36 项 API 断言、浏览器实际收款/再次编辑、视图回退及专项检查通过。既有审计请求 401 另列 P1；未生产迁移或部署。分析、设计、回退、复盘及下一步见 2.0 开发文档 M3-04 第二批与 evidence/20260905-m3-04-verification.md。

---


## 2026-09-16 门票与咖啡产品增加成本价维护

**需求编号**：OPS-COST-20260916
**状态**：已发布待观察

**分析**

产品管理中的门票和咖啡配置仅包含名称与售价，无法维护单位成本。配置数据已保存在 `app_config.value` JSON 中，增加 `costPrice` 不需要改表或执行数据库迁移。

**执行**

- 门票、咖啡列表增加“成本价 / 售价”列，旧配置未提供成本时按 `0.00` 显示。
- 门票、咖啡新增/编辑弹窗增加成本价输入与非负数校验，保存为 `costPrice`。
- 工坊配置保持原有名称和单价字段，不扩大范围。
- 新建数据库种子配置补充 `costPrice: 0`，现有数据不做批量回填。
- 更新 `models.js` / `ui.js` cache-bust token，同步应用内 `dist/` 和根目录 `dist/` 镜像。

**发布**

- 发布时间：2026-09-16 12:34 +0800。
- 发布版本：`2.0.0-rc.3`，cache-bust token 为 `product-cost-20260916`。
- 上传文件：`/opt/aiwei/app/index.html`、`/opt/aiwei/app/js/models.js`、`/opt/aiwei/app/js/ui.js`。
- 回滚备份：`/opt/aiwei/backups/product-cost-20260916-20260916-123437`。

**验证**

- `node --check app/js/ui.js` 通过。
- `node --check app/js/models.js` 通过。
- `npm run check:product-config-cost-browser` 通过：覆盖门票/咖啡列表、旧配置默认 0、编辑保存持久化参数和工坊隔离。
- `npm run check:pos-ticket-quantity-browser` 通过，票务数量与收银合计逻辑未回退。
- `npm run check:dist-sync` 通过；根目录 `dist/` 三个相关文件 SHA-256 与应用源文件一致。
- HTTPS 首页返回 200 并命中 `product-cost-20260916`；`models.js` 和 `ui.js` 返回 200 并命中成本字段标识。
- 远端三个文件 SHA-256 与本地一致；nginx、API、数据库容器健康，API 内部 `/healthz` 返回 `{"status":"ok"}`，近 80 行日志中 `SyntaxError` / `ReferenceError` 为 0。
- 未登录请求 `revenue` 返回 401，符合当前鉴权口径。本地保存的 `test4` 旧测试凭据已失效，未完成登录后人工页面验收，列为发布后观察项。

**回滚与边界**

- 回滚仅需从上述备份恢复三个前端文件；未执行数据库迁移，无不可逆风险。
- 本次未改历史销售、未将成本写入历史交易快照、未改权限或后端。
...
## 2026-09-06 M3-07 画廊销售流程升级本地交付

版本 2.0.0-dev.m3-07.1。画廊新销售必须明确关联已上架作品，保存成交时的结算价、零售价、总额和净额快照；销售、库存、现金、退款/作废及服务端审计由同一事务处理。列表新增结算成本与贡献额，移除物理删除入口。11 项 HTTP 检查、失败回退、快照冻结、退款权限与审计、迁移回滚/幂等、桌面和手机实际录入通过。两处 dist 同步；未生产迁移或部署。详见 2.0 开发文档 M3-07 与 evidence/20260906-m3-07-verification.md。下一开发任务 M3-08。
## 2026-09-06 M3-08 空间与合作项目流程本地交付

版本 2.0.0-dev.m3-08.1。空间使用升级为包含业务类型、合作方式、经营状态、真实合同编号和项目负责人的合作项目；默认映射允许人工确认。项目保存与分期到账改为服务端事务校验和可信审计，保持按实际到账日确认收入，拦截时间冲突与超额到账。14 项 HTTP/SQL 检查、失败回退、迁移回滚/幂等及 Edge 桌面和手机实际保存/收款通过。未生产迁移或部署。详见 evidence/20260906-m3-08-verification.md。下一开发任务 M3-09。
## 2026-09-06 M3-09 2.0 只读事实与基础汇总本地交付

版本 2.0.0-dev.m3-09.1。数据看板新增四业务层净收入、销售成本、期间成本、销售毛利与经营贡献，并与原 1.0 净收入并行显示差异；治理清单定位待归类收入、待归类成本和缺成本商品/作品。11 项 HTTP/SQL 检查、只读权限、成本边界、迁移回滚/幂等、Edge 桌面和手机页面及既有专项回归通过。未生产迁移或部署。详见 evidence/20260906-m3-09-verification.md。下一任务 M3-10。

## 2026-09-08 M3-10 M3 验证与阶段评审

版本 2.0.0-dev.m3-10.1。M3 本地阶段有条件通过：支出、工坊、画廊、空间和管理汇总共 85 项集成断言，迁移回滚/幂等、1.0 日结兼容、v2 只读事实和权限边界均通过。同时关闭通用 CRUD 操作日志 401 问题，由服务端按登录身份写入可信审计。允许进入 M4/M5/M6 本地开发；生产迁移、真实数据对账、历史治理和上线仍需单独授权。详见 evidence/20260908-m3-10-stage-review.md。下一任务 M4-01。

## 2026-09-08 M4-01 数据治理任务拆分与冻结

M4 已按治理基线、商品别名、商品成本证据、收入/支出归属、画廊/工坊弱关联、空间历史分类、分批 dry-run 和阶段评审拆为 M4-02 至 M4-10。冻结“不覆盖原始流水、歧义不自动归类、当前成本不无依据倒推历史、分批可预览/审计/撤销”的安全边界。本任务未连接生产库、未回填历史数据、未部署。详见 evidence/20260908-m4-01-task-breakdown.md。下一任务 M4-02。

## 2026-09-08 M4-02 治理基线与稳定问题标识

版本 2.0.0-dev.m4-02.1。治理明细新增由问题类型和原始来源确定的稳定 ID、问题分组与优先级；新增按月份/类型/来源汇总的只读基线视图。数据看板可查看问题数和影响金额，并导出含稳定键和原始定位的 CSV。本地 6 条测试问题的唯一性/稳定性、基线对账、迁移回滚/幂等、8 项 HTTP 检查、Edge 桌面/手机和 CSV 下载均通过。未读取生产、未回填、未部署。详见 evidence/20260908-m4-02-governance-baseline.md。下一任务 M4-03。

## 2026-09-08 M4-03 历史商品别名候选

版本 2.0.0-dev.m4-03.1。新增历史零售别名只读候选视图，按标准化历史名和售价聚合，区分已有匹配、唯一候选、歧义和无候选四种状态；售价只在精确同名商品间辅助消歧，不允许价格单独决定商品或饮料类型。数据看板支持全历史候选汇总、状态筛选和 CSV 导出。四状态样本、稳定 ID、同价不同名保护、迁移回滚/幂等、来源事实不变、8 项 HTTP 检查及 Edge 桌面/手机通过。未读取生产、未创建别名、未回填、未部署。详见 evidence/20260908-m4-03-product-alias-candidates.md。下一任务 M4-04。

## 2026-09-08 M4-04 商品业务分类与成本证据补全

版本 2.0.0-dev.m4-04.1。新增商品成本证据与商品主数据治理两张只读视图，区分成交成本快照、需确认期间的别名成本和只从更新时间起参考的当前成本；仅唯一成交快照成本可作为历史观察建议，当前成本不倒推。数据看板展示分类缺口、成本证据覆盖、未验证历史明细和证据来源，支持状态筛选与 CSV。迁移回滚/幂等、来源事实不变、9 项 HTTP 检查、Edge 桌面/手机及 M4-02/M4-03/M3 回归通过。未读取生产、未修改商品或历史流水、未部署。详见 evidence/20260908-m4-04-product-cost-evidence.md。下一任务 M4-05。

## 2026-09-08 M4-05 历史收入归属复核

版本 2.0.0-dev.m4-05.1。新增未归类收入只读候选视图，人工链接优先于规则；自动候选只比较最高优先级规则，同级规则指向不同业务类型时保留冲突，低优先级规则不覆盖结论。数据看板按年/月展示已有人工归属、唯一规则、冲突、无候选和影响金额，支持状态筛选与 CSV。五类样本、安全正则、迁移回滚/幂等、来源数据不变、8 项 HTTP 检查、Edge 桌面/手机及 M4-02 至 M4-04、M3 回归通过。未读取生产、未创建规则或链接、未回填、未部署。详见 evidence/20260908-m4-05-revenue-attribution.md。下一任务 M4-06。

## 2026-09-08 M4-06 历史支出与成本归属复核

版本 2.0.0-dev.m4-06.1。新增历史支出只读候选视图，保留原类型、项目和类别；人工链接优先，最高优先级规则分别复核成本类型、业务层、业务类型和能力轴，部分维度、同级冲突与无候选均留待人工确认。项目登记只做精确同名候选，区分唯一、同名冲突、未登记及共享支出。迁移回滚/幂等、来源数据不变、9 项 HTTP、Edge 桌面/手机及 M4-02 至 M4-05、M3 关键回归通过。未读取生产、未修改支出/规则/链接/项目、未回填、未部署。详见 evidence/20260908-m4-06-cost-attribution.md。下一任务 M4-07。

## 2026-09-09 M4-07 画廊与工坊历史弱关联治理

版本 2.0.0-dev.m4-07.1。新增画廊作品/结算证据与工坊项目/直接成本两张只读候选视图。画廊按作品 ID、编号、名称加艺术家和名称精确分级，成交快照优先，当前结算价仅作参考；工坊按项目精确同名及同月同项目支出生成候选，确认成本与待审成本分开，不估算或跨月分摊。11 项 HTTP、迁移回滚/幂等、来源数据不变、两份 CSV、Edge 桌面/手机及 M3-06/M3-07、M4-03 至 M4-06 回归通过。未读取生产、未回填、未部署。详见 evidence/20260909-m4-07-gallery-workshop-links.md。下一任务 M4-08。

## 2026-09-09 M4-08 空间与合作历史分类治理

版本 2.0.0-dev.m4-08.1。新增空间历史分类只读候选视图，分别复核业务类型、合作方式、合同编号和经营状态；明确租赁及品牌快闪可给出建议，会议、团建、沙龙和展览保留混合用途人工判断。合同检查覆盖付费项目缺失和编号重复，页面展示应收、已收、待收及证据，支持状态筛选和 CSV。8 类迁移样本、10 项 HTTP、回滚/幂等、来源数据不变、Edge 桌面/手机、M3-08 及 M4-02 至 M4-07 回归通过。未读取生产、未回填、未部署。详见 evidence/20260909-m4-08-space-classification.md。下一任务 M4-09。

## 2026-09-09 M4-09 分批应用与 dry-run 安全工具

版本 2.0.0-dev.m4-09.1。新增治理批次、项目、事件和汇总视图，首批只应用收入/成本唯一规则候选到可撤销的业务解释层。草稿必须先 dry-run，随后提交并由管理员批准；应用和撤销均在事务内核对前后值，审批后发生并发变化时整批回滚。管理页支持创建、预览、审批、应用、撤销和 CSV 影响报告。迁移回滚/幂等、27 项 HTTP、批准/拒绝、完整审计、原始事实不变、Edge 桌面/手机及 M4-05/M4-06/M4-08、M3 关键回归通过。未读取生产、未执行真实批次、未部署。详见 evidence/20260909-m4-09-governance-batches.md。下一任务 M4-10。

## 2026-09-09 M4-10 M4 验证与阶段评审

版本 2.0.0-dev.m4-10.1。M4 本地阶段有条件通过：七类治理候选在隔离样本中覆盖 41/41，治理基线 19 个问题与 9 个分组完全对账；八组迁移回滚/幂等、47 项阶段 HTTP、27 项批次接口、Edge 桌面/手机、M3 和 1.0 关键回归均通过。歧义、无候选、部分规则、当前成本倒推和混合用途仍保留人工判断；阶段批次完整验证审批、可信审计、原子应用与撤销，收入、支出、画廊、空间、收款和日结核心金额不变。允许进入 M5/M6 本地设计与开发；生产读取、真实覆盖率、历史治理、线上对账、部署和切换仍需单独授权。详见 evidence/20260909-m4-10-stage-review.md。下一任务建议 M5-01。

## 2026-09-09 M5-01 运营管理页面任务拆分与冻结

M5 已拆分为 M5-02 至 M5-09 八个任务包，按“入口框架→四层总览→趋势比较→治理工作台→模型对应→筛选下钻导出→韧性性能→阶段评审”执行。冻结独立“美术馆运营管理”入口、七层页面信息架构、九类数据源映射及 admin/editor 可读、viewer 暂不开放敏感经营信息的权限边界。页面只复用 2.0 事实、汇总、治理候选和业务维度，不复制数据看板口径，不增加绕过 M4 审批的写入口。未连接生产、未部署。详见 evidence/20260909-m5-01-task-breakdown.md。下一任务 M5-02。

## 2026-09-09 M5-02 运营管理入口与页面框架

版本 2.0.0-dev.m5-02.1。新增仅 admin/editor 可见的“运营管理”独立入口和页面模块，包含期间/数据范围上下文、年月筛选、刷新状态、分区导航及经营总览、四层矩阵、趋势比较、治理工作台、模型对应、明细导出六个内容区。页面模块从 `ui.js` 分离，当前只建立结构，不提前请求经营接口或伪造指标。三套静态文件已同步；JS、缓存标识、Edge 桌面/手机、admin/editor/viewer 权限及年月/分区/刷新交互通过，页面错误为 0。未读取生产、未改数据库、未部署。详见 evidence/20260909-m5-02-operations-shell.md。下一任务 M5-03。

## 2026-09-09 M5-03 四层经营总览

版本 2.0.0-dev.m5-03.1。运营管理页按选定月份读取 `business_layer_summary_v2` 与 `data_governance_baseline_v2`，展示六项总体指标和固定四层经营矩阵。金额直接使用冻结汇总字段；待归类收入/成本单列且不并入四层，零值层保留，净收入为零时毛利率显示“—”。页面区分加载、空期间和接口失败，服务端期间过滤、admin/editor/viewer 权限和只读保护保持有效。纯模型、静态同步、既有前端回归及 Edge 桌面/390px 手机验收通过，隔离样本精确对账并已清理。未读取生产、未改数据库、未部署。详见 evidence/20260909-m5-03-operations-overview.md。下一任务 M5-04。

## 2026-09-10 M5-04 月度趋势与口径比较

版本 2.0.0-dev.m5-04.1。运营管理页新增固定 12 个月的 2.0 净收入、总成本、销售毛利和经营贡献趋势，并用同年度 `revenue_facts` 展示 1.0/2.0 净收入及“2.0 减 1.0”差额。年度卡片、当前月份比较、两张图表和月度明细使用服务端年度过滤；经营指标直接汇总冻结视图字段，未在前端建立另一套会计公式。趋势区分空年份和接口失败，失败不清空经营总览。纯模型、Edge 桌面/390px 手机、权限、只读保护及 M5-02/M5-03 回归通过，隔离样本已清理。未读取生产、未改数据库、未部署。详见 evidence/20260910-m5-04-operations-trend.md。下一任务 M5-05。

## 2026-09-10 M5-05 数据治理工作台

版本 2.0.0-dev.m5-05.1。运营管理页按所选月份展示治理问题基线、P1-P4 队列、商品与成本等六个候选域以及治理批次状态，复用 M4 的九个只读治理来源并限制每类最多读取 500 条。单个来源失败只降级对应卡片。补齐 editor 进入 M4 受控批次页的入口，editor 只看到批次区和草稿/dry-run/提交动作，批准、拒绝、应用、撤销仍仅限 admin，运营管理工作台保持只读。纯模型、Edge 桌面/390px 手机、空期间、失败隔离、三角色权限及 M5-02 至 M5-04 回归通过，隔离样本已清理。未读取生产、未改数据库、未部署。详见 evidence/20260910-m5-05-governance-workbench.md。下一任务 M5-06。

## 2026-09-10 M5-06 业务模型对应展示

版本 2.0.0-dev.m5-06.1。运营管理页只读展示 `business_dimensions` 的业务层、业务类型、成本类型和能力轴四类冻结字典，业务类型标明所属业务层，启用/停用状态保持可见；新增经营事实链和净收入、成本、毛利、贡献、治理影响及 1.0/2.0 差异共 9 项页面指标映射。模型说明全局生效、不随年月改变，不提供写入口；接口失败只降级模型区。隔离库 28 个字典项精确对账，纯模型、Edge 1440/390px、权限、失败隔离、静态同步及既有关键回归通过。未读取生产、未改数据库、未部署。详见 evidence/20260910-m5-06-business-model-map.md。下一任务 M5-07。

## 2026-09-10 M5-07 统一筛选、来源下钻与导出

版本 2.0.0-dev.m5-07.1。运营管理页接入收入、成本和利润三张只读事实视图，统一年月、业务层和映射状态使用服务端条件，关键词在最多 500 条结果内定位；明细展开保留事实 ID、来源表、来源 ID、明细键、四类业务维度、映射状态和质量标记。CSV 复用当前筛选并使用 UTF-8 BOM，保留来源三元组和冻结金额字段。隔离样本的净收入 ¥160.00、成本 ¥35.00、销售毛利 ¥140.00 与接口精确一致，Edge 1440/390px、空值、失败隔离、权限、下载内容及 M5-03 至 M5-06 回归通过。测试数据已清理，未读取生产、未改数据库、未部署。详见 evidence/20260910-m5-07-detail-export.md。下一任务 M5-08。

## 2026-09-10 M5-08 页面韧性与性能优化

版本 2.0.0-dev.m5-08.1。运营管理五个数据分区新增可取消请求、过期响应保护、`aria-busy` 和同范围成功数据保留；同年月份切换复用年度趋势，全局复用业务字典、商品别名和商品治理候选，手动刷新强制更新。通用 REST GET 新增兼容的 `count=none`，页面初始数据库查询由最多 30 次降为 15 次，同年月份切换降至 10 次，所有列表上限不变。专项 Edge 验证覆盖请求计数、缓存、强制刷新、故障注入、1440/390px 和减少动态效果，M5-03 至 M5-07 及既有前端回归通过。隔离数据已清理，未读取生产、未改业务口径、未部署。详见 evidence/20260910-m5-08-resilience-performance.md。下一任务 M5-09。

## 2026-09-10 M5-09 M5 阶段评审

版本 2.0.0-dev.m5-09.1。M5 本地阶段有条件通过：66 项阶段 HTTP 检查核对 15 个敏感视图的三角色权限与只读保护，16 条隔离汇总行的金额和比率公式、28 个业务字典项和治理基线对账均通过；M5-03 至 M5-08 六组 Edge 1440/390px 、故障隔离、下钻导出和查询优化回归通过，页面异常为 0。M3 管理汇总 11 项与 M4 受控批次 27 项关键 HTTP 回归通过，测试前后原始事实不变。评审发现并修正了专用测试库漏应用 M4-06 至 M4-09 的环境漂移，M6 需固定从新库重放完整迁移链。允许进入 M6-01 本地方案拆分；生产对账、历史治理、备份恢复演练、部署、灰度和切换未开始。详见 evidence/20260910-m5-09-stage-review.md。下一任务 M6-01。

## 2026-09-10 M6-01 验证、试运行与切换任务拆分

版本 2.0.0-dev.m6-01.1。M6 拆分为 M6-02 至 M6-10 九个任务包，按“全新隔离库重放→代码/安全回归→1.0/2.0 对账→灰度开关→试运行观测→生产前置包→生产追加迁移→内部试运行→切换评审”执行。冻结金额 0.01 元精确对账、差异解释集、`off/admin/staff` 三档开关、G0-G4 门槛、P0-P3 问题分级和生产授权矩阵。M6-02 至 M6-07 可在本地继续；M6-08 起的生产读取、备份、迁移、部署、灰度和历史治理不在本任务授权中。详见 evidence/20260910-m6-01-task-breakdown.md。下一任务 M6-02。

## 2026-09-10 M6-02 完整迁移链与新库重放

版本 2.0.0-dev.m6-02.1。固化 2.0 前历史初始化基线、Git blob 和逐文件 SHA-256 清单，在全新本地 PostgreSQL 临时库完成 18 个正向迁移、18 个文件级幂等重放、14 个逆向回滚及回滚后的完整正向恢复。首次演练发现并补齐 1.0 商品/作品审批字段前置迁移；28 个业务维度、7 张核心表、18 个 2.0 视图、结构指纹和旧版事实逐字段验证通过。临时库已删除，未连接生产、未部署。详见 evidence/20260910-m6-02-fresh-database-rehearsal.md。下一任务 M6-03。

## 2026-09-11 M6-03 代码、权限与核心流程回归

版本 2.0.0-dev.m6-03.1。新增 100 项 M6 四角色/核心流程 HTTP 检查，连同支出、工坊、画廊、空间及 M3-M5 阶段共完成 312 项 HTTP 断言；91 个一方 JS、静态同步、M4 八组迁移专项及 M4/M5 八组 Edge 桌面/手机页面回归通过。修正公共请求的 Node `url.parse()` 安全弃用警告并保留重复参数语义，M4 阶段浏览器版本断言改读当前 `VERSION`；测试数据完整清理，原始事实不变。无未关闭 P0/P1，未连接生产、未部署。详见 evidence/20260911-m6-03-code-permissions-core-regression.md。下一任务 M6-04。

## 2026-09-11 M6-04 1.0/2.0 自动对账

版本 2.0.0-dev.m6-04.1。新增按分精确比较的自动对账脚本，覆盖收入逐来源/日/月/业务层、期间支出、零售和画廊销售成本、画廊成交/退款/结算、空间应收/到账及日结。样本 1.0 净收入 867.00 元 = 2.0 已归类 910.00 元 + 待归类 -43.00 元；POS、画廊、空间和退款逐来源差异均为 0.00 元。差异集定位到 `m604-pos/other_amount` +7.00 元和 `m604-refund/adjustment` -50.00 元，均有依据；无未解释金额。测试数据已清理，未连接生产、未部署。详见 evidence/20260911-m6-04-reconciliation.md。下一任务 M6-05。

## 2026-09-11 M6-05 2.0 灰度开关与入口控制

版本 2.0.0-dev.m6-05.1。新增默认关闭的 `off/admin/staff` 三档运营管理灰度开关：管理员在数据管理页填写原因后切换，服务端以事务锁、角色校验和可信日志记录旧值、新值、操作人及时间；通用配置接口不能绕过。侧栏入口和页面渲染双重校验，开放时显示试运行标识，viewer 始终不可访问敏感运营页；关闭不影响 1.0。26 项 HTTP、Edge 三角色/三模式、19 正向/15 回滚新库重放、M6 核心 100 项和自动对账回归通过。未连接生产、未部署、未推送。详见 evidence/20260911-m6-05-rollout-control.md。下一任务 M6-06。

## 2026-09-11 M6-06 试运行观测与问题闭环

版本 2.0.0-dev.m6-06.1。管理页新增 admin/editor 可读的版本、数据库、迁移、灰度、API 错误与 P95 耗时观测面；服务端保留最近 500 个无请求体/令牌的运行指标并记录 5xx/慢请求。新增 P0–P3 持久问题台账及事件链，覆盖证据、影响、负责人、修正版本和管理员复验，P0/P1 在 `verified` 前持续阻断 G0。23 项 HTTP、Edge 闭环、20 正向/16 回滚新库重放、M6-03 至 M6-05 关键回归通过，原始事实不变；发布包依赖补齐。G0 本地发布候选门槛达标，未连接生产、未部署、未推送。详见 evidence/20260911-m6-06-observability-issue-loop.md。下一任务 M6-07。

## 2026-09-11 M6-07 生产前置包与恢复演练

版本 2.0.0-dev.m6-07.1。备份增加结构/业务基线 JSON 及双 SHA，发布前置检查核对依赖、静态同步和 20 正向/16 回滚迁移；同一 260,506-byte PostgreSQL 17 custom dump 两次隔离恢复均与源库基线一致，恢复点后探针被清除，源库不变且临时库无残留。修正部署组包/版本注入顺序，新增数据库 `/healthz` 和强制就绪失败，运行手册与 Go/No-Go 清单已固化。G1 通过；未连接生产、未部署、未推送。详见 evidence/20260911-m6-07-production-readiness-restore.md。下一任务 M6-08，生产操作需单独授权。

## 2026-09-11 M6-08 生产追加迁移与只读校验

版本 2.0.0-dev.m6-08.1。生产备份双 SHA、对象清单和本地隔离恢复通过；针对 1.3.3 遗留 `space_usage.received_amount` 修正 M4-08 字段歧义，并在生产备份形态重放全链后恢复执行。生产现为 28 表/21 视图、28 个维度，核心事实前后不变，19 个 2.0 视图与真实数据对账通过。API/静态已部署，公网 HTTPS 200、未登录 API 401、容器健康、灰度 `off`。1 个历史空间超收和 `xlsx` high 告警列为 P2。G2 通过；详见 evidence/20260911-m6-08-production-migration-validation.md。下一任务 M6-09，开启生产灰度需单独授权。

## 2026-09-13 M6-09 内部灰度试运行与修正

试运行版本保持 `2.0.0-dev.m6-08.1`。灰度前发现 09-12、09-13 定时备份因 Shell 执行权限丢失失败，入口保持 `off`，按 P1 完成权限恢复、部署防复发修正、新备份、双 SHA、对象清单和本地 PostgreSQL 17 隔离恢复，问题经管理员复验关闭。随后生产按 `off → admin → staff` 留痕开放；两阶段多轮 18 个敏感视图、角色边界、真实金额和运行观测通过，0 个 5xx、0 个未关闭 P0/P1。收入 `194,387.60 - 68.00 = 194,319.60`、期间支出 4,461.80 元精确对账，画廊/空间/日结保存公式无差异。4 个 P2 已登记并设负责人/期限：空间超收 1 行、`xlsx` high、两次经营汇总慢请求、2 个已确认日结日期存在闭账后新增收入。G3 通过，保留 `staff` 内部灰度，viewer 不开放；详见 evidence/20260913-m6-09-internal-trial.md。下一任务 M6-10，正式切换仍需馆长确认。

## 2026-09-13 M6-10 M6 阶段评审与切换决策

生产版本保持 `2.0.0-dev.m6-08.1`，未重新部署。M6-02 至 M6-09 技术证据复核通过；生产最新 36 次敏感视图读取无失败，累计 544 个请求无 5xx、P95 228.5 ms，收入/支出及画廊、空间、日结公式继续对账，`staff` 角色矩阵正确，未关闭 P0/P1 为 0。代码回退包、当前可恢复备份和一键 `off` 均可用。4 个 P2 不阻断内部灰度。由于权限修复后的 09-14 首次自动备份尚未发生，且真实 admin/editor 业务验收和馆长签字尚缺，当前建议继续 `staff` 内部灰度，不宣布正式切换。技术决策包和业务验收清单已形成；G4 待馆长确认。详见 evidence/20260913-m6-10-stage-review-switch-decision.md。

## 2026-09-13 2.0.0-rc.1 生产候选版本发布

版本更新为 `2.0.0-rc.1`，提交 `e4a28c0` 已推送 GitHub `codex/operations-v2` 并部署腾讯云。部署前生成 380,127-byte 数据库 custom dump、双 SHA、基线和 manifest，服务器保留 4,293-byte 代码回退包；本次未执行数据库迁移、未改变 `staff` 灰度。部署后容器健康，公网首页 200、未登录敏感接口 401；36 个受保护视图读取无失败，admin/editor 开放、viewer 隔离，收入与期间支出差额均为 0，运行观测无 5xx、无未关闭 P0/P1。该版本用于内部业务验收，G4 正式切换仍待自动备份复核、真实业务验收和馆长确认。详见 evidence/20260913-2.0.0-rc.1-cloud-deployment.md。

## 2026-09-13 UAT-01 文创零售自动检索优化

版本 `2.0.0-rc.2`。收银台产品名输入框新增已上架商品实时候选，覆盖名称、标准名、规格、条码、SKU、供应商和启用别名，并以顺序模糊匹配支持“可乐 → 可口可乐”；选择后自动回填商品名、零售价和数量，保留商品快照，无结果仍可手动输入。浏览器验证连续检索、别名、回车、价格回填、无匹配、原弹窗兼容及 390px 手机宽度，零售快照和三套静态镜像通过。提交 `7277c53` 已推送和部署；生产对应商品已上架、售价 4.00 元、库存 100，HTTPS 资源、容器健康、36 个受保护视图和角色矩阵通过。未改数据库、业务数据或权限；回滚目录为 `/opt/aiwei/backups/frontend-pos-autocomplete-20260913-154903`。详见 evidence/20260913-uat-01-pos-retail-autocomplete.md。
## 2026-09-14 UAT-02 门票与套票数量直接录入

版本 `2.0.0-rc.3`。普通票与套票商品卡增加数字输入框，可直接键入数量，同时保留加减按钮；所有入口复用同一数量更新逻辑，数量限制为不小于 0 的整数，单项小计与收银总额实时同步。编辑历史收入时同步回填数量和卡片小计。咖啡商品继续使用原有加减操作。

浏览器专项覆盖普通票、套票、加号同步、负数、小数、编辑回填、咖啡隔离和 390px 手机宽度；文创自动检索、零售快照、静态同步和 M6 发布检查通过。提交 `5a4da60` 已部署腾讯云 `2.0.0-rc.3`；公网新资源、API 健康、未登录 401、36 个受保护读取和 `staff` 角色矩阵通过。无数据库、业务数据和权限变更；回滚目录 `/opt/aiwei/backups/frontend-ticket-quantity-20260914-135116`。详见 `evidence/20260914-uat-02-pos-ticket-quantity-entry.md`。
## 2026-09-16 工坊产品增加成本价维护

**需求编号**：OPS-COST-20260916-B
**状态**：已发布待观察

**改动**

- 工坊产品列表增加“成本价 / 售价”列，旧配置未提供成本时按 `0.00` 显示。
- 工坊新增/编辑弹窗增加成本价输入和非负数校验，保存为 `costPrice`。
- 默认工坊配置和新建数据库种子配置补充 `costPrice: 0`。
- 保留现有工坊活动直接成本/支出归属口径；不改历史流水，不回填历史成本。
- cache-bust token 更新为 `workshop-cost-20260916`。

**发布**

- 发布时间：2026-09-16 13:26 +0800。
- 发布版本：`2.0.0-rc.3`，上传 `index.html`、`js/models.js`、`js/ui.js`。
- 回滚备份：`/opt/aiwei/backups/workshop-cost-20260916-20260916-132609`。

**验证与边界**

- `npm run check:product-config-cost-browser` 通过：覆盖门票、咖啡、工坊的成本列、旧配置默认 0 和编辑保存。
- `npm run check:workshop-projects` 通过：工坊项目、活动类型、参与人数、金额快照和旧数据兼容未回退。
- `npm run check:pos-ticket-quantity-browser` 和 `npm run check:dist-sync` 通过，根目录 `dist/` 同步。
- HTTPS 首页和两个 JS 资源均返回 200，命中 `workshop-cost-20260916` 及工坊成本字段标识。
- 远端三个文件 SHA-256 与本地一致；nginx、API、数据库容器健康，API 内部 `/healthz` 正常，近 80 行日志无 `SyntaxError` / `ReferenceError`。
- 未登录 `revenue` 返回 401，符合鉴权口径；由于既有 `test4` 凭据已失效，登录后人工页面验收仍为发布后观察项。
- 本次无数据库迁移、无生产数据写入、无权限变更。
