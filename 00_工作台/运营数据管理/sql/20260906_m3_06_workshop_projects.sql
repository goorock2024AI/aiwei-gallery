-- M3-06: run after M3-04 and M3-05. No raw rows are rewritten.
-- Workshop line snapshots become project-aware; costs remain separate facts.
...
workshop_lines AS (
  SELECT
    ...
    COALESCE(NULLIF(item.value->>'activity_type_code', ''), NULLIF(item.value->>'activityTypeCode', ''), 'workshop') AS activity_type_code,
    CASE
      WHEN COALESCE(item.value->>'qty', item.value->>'quantity', item.value->>'count', '') ~ '^-?[0-9]+(\.[0-9]+)?$'
        THEN COALESCE(item.value->>'qty', item.value->>'quantity', item.value->>'count')::NUMERIC
      ELSE 1
    ...
    END AS line_amount,
    r.payment_method,
    COALESCE(NULLIF(item.value->>'project_name', ''), NULLIF(item.value->>'projectName', ''), NULLIF(r.project_name, ''), '') AS project_name,
    r.handler,
    r.created_at
  FROM revenue r
  CROSS JOIN LATERAL jsonb_array_elements(
...
raw_revenue_facts AS (
  ...
  SELECT
    ...
    COALESCE(NULLIF(link.business_type_code, ''), NULLIF(wl.activity_type_code, ''), 'workshop'),
    ...
    jsonb_strip_nulls(jsonb_build_object(
      'missing_project', CASE WHEN COALESCE(wl.project_name, '') = '' THEN true ELSE NULL END
    )),
    wl.created_at
...
DROP VIEW IF EXISTS workshop_project_performance_v2;
CREATE VIEW workshop_project_performance_v2 AS
WITH revenue_by_project AS (
  SELECT
    left(business_date, 7) AS activity_month,
    lower(trim(project_name)) AS project_key,
    min(trim(project_name)) AS project_name,
    min(business_date) AS first_activity_date,
    max(business_date) AS last_activity_date,
    string_agg(DISTINCT business_type_name, ' / ' ORDER BY business_type_name) AS activity_types,
    sum(quantity)::NUMERIC AS participant_count,
    sum(net_amount)::NUMERIC AS revenue_amount,
    count(DISTINCT source_id)::INTEGER AS revenue_record_count,
    bool_or(COALESCE((quality_flags->>'missing_project')::BOOLEAN, false)) AS has_missing_project
  FROM business_revenue_facts_v2
  WHERE business_layer_code = 'experience_activity'
    AND business_type_code IN ('workshop', 'course_study')
    AND COALESCE(trim(project_name), '') <> ''
  GROUP BY left(business_date, 7), lower(trim(project_name))
),
cost_by_project AS (
  SELECT
    left(business_date, 7) AS activity_month,
    lower(trim(project_name)) AS project_key,
    sum(cost_amount)::NUMERIC AS direct_cost_amount,
    count(*)::INTEGER AS cost_record_count,
    count(*) FILTER (WHERE cost_type_code = 'uncategorized_cost')::INTEGER AS pending_cost_count
  FROM business_cost_facts_v2
  WHERE cost_basis = 'period_expense'
...
SELECT
  ...
  r.participant_count,
  r.revenue_amount,
  COALESCE(c.direct_cost_amount, 0)::NUMERIC AS direct_cost_amount,
  (r.revenue_amount - COALESCE(c.direct_cost_amount, 0))::NUMERIC AS contribution_amount,
  CASE WHEN r.revenue_amount > 0
    THEN round((r.revenue_amount - COALESCE(c.direct_cost_amount, 0)) / r.revenue_amount, 4)
    ELSE NULL END AS contribution_margin,
  r.revenue_record_count,
  COALESCE(c.cost_record_count, 0)::INTEGER AS cost_record_count,
  COALESCE(c.pending_cost_count, 0)::INTEGER AS pending_cost_count,
  (COALESCE(c.cost_record_count, 0) = 0) AS missing_direct_cost,
  r.has_missing_project
FROM revenue_by_project r
LEFT JOIN cost_by_project c USING (activity_month, project_key);
