...
所有系统任务先做初步分级和路由，再进入执行闭环。方向型任务（如优化、重设计、提升体验、流程改造）必须先通过问答确认真实目标，不能把用户原话直接当作任务目标。

执行闭环见 [CLAUDE.md](CLAUDE.md) 与 `docs/agents/six-step-loop.md`；复盘和继续发现保留，但按任务等级控制记录长度。
